...
The mean number of steps taken per day is `r meanperday` and the median is
`r medianperday` steps per day.
...
The maximum average number of steps per interval of `r maxval` occurs at interval number `r intervalofmax` which is the 5-minute interval following `r maxtime`.
...
```
1. There are `r numna` missing values out of the `r numintervals` in the dataset, hence for a percentage of `r percna` % of the intervals no data was recorded. 
...
The mean number of steps take per day is now `r meanperdaynona`, exactly what it was before, and the median is now `r medianperdaynona` steps per day.
...
The number of NA's in the dataset should therefore equal 8 days times 288 intervals per day, which is indeed `r numna`, the number reported above.  

The upshot is that my imputation strategy would have added eight days with a total of `r meanperdaynona` steps, which is the original mean number of steps per day.
